...
*
* @returns FRE_OK
*          FRE_INVALID_ARGUMENT if strTrace is null
*          FRE_WRONG_THREAD
*/

FREResult FRETrace(FREContext ctx, const char* strTrace);

typedef enum {
	FRE_RENDERMODE_UNKNOWN = 0,
	FRE_RENDERMODE_NONE,
	FRE_RENDERMODE_CPU,
	FRE_RENDERMODE_DIRECT_OGLES,
	FRE_RENDERMODE_DIRECT_OGL,
	FRE_RENDERMODE_DIRECT_D3D9,
	FRE_RENDERMODE_DIRECT_D3D10,
	FRE_RENDERMODE_DIRECT_D3D11,
	FRE_RENDERMODE_SOFTWARE_GDI,
	FRE_RENDERMODE_GPU_OGLES,
	FRE_RENDERMODE_ENUMPADDING = 0xfffff /* will ensure that C and C++ treat this enum as the same size. */
} FRERenderMode;
...
*
* @returns FRE_OK
*          FRE_INVALID_ARGUMENT if pRenderMode is null
*          FRE_INVALID_OBJECT if stage is non-null but not a Stage object
*          FRE_WRONG_THREAD
*/

FREResult FREGetRenderMode(FREContext ctx, FREObject stage, uint8_t* pRenderMode);
...
*
* @returns FRE_OK
*          FRE_INVALID_ARGUMENT if either element is null
*          FRE_INVALID_OBJECT if the objects are incorrect types
*          FRE_WRONG_THREAD
*/

FREResult FRESetRenderSource(FREContext ctx, FREObject source, FREObject target);
...
*
* @returns FRE_OK
*          FRE_INVALID_ARGUMENT if any parameter is null
*          FRE_INVALID_OBJECT if the MediaBuffer objects has an incorrect type
*          FRE_WRONG_THREAD if called from the wrong thread; at some point this function will be updated to allow multi-thread calling
*/

FREResult FREMediaBufferLock(FREContext ctx, FREObject mediaBuffer, uint8_t** pData, uint32_t* pWidth, uint32_t* pHeight, uint32_t* pStride, uint32_t* pFormat);
...
*
* @returns FRE_OK
*          FRE_INVALID_ARGUMENT if the context or mediaBuffer parameter is null
*          FRE_INVALID_OBJECT if the MediaBuffer objects has an incorrect type
*          FRE_WRONG_THREAD if called from the wrong thread; at some point this function will be updated to allow multi-thread calling
*/

FREResult FREMediaBufferUnlock(FREContext ctx, FREObject mediaBuffer, uint32_t bUpdate);
...
/**
 * Release data that has been acquired with an earlier call to FREAcquireBitmapData[2]
 *
 * @return  FRE_OK
 *          FRE_WRONG_THREAD
 *          FRE_ILLEGAL_STATE
